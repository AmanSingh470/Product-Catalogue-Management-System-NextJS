...
import AddDivisionModal from "@/components/Admin/Division/Modal/AddDivisionModal";
import EditDivisionModal from "@/components/Admin/Division/Modal/EditDivisionModal";
import DeleteDivisionModal from "@/components/Admin/Division/Modal/DeleteDivisionModal";
...
export default function DivisionTable() {
    ...
    const Divisions = Array.from({ length: 1284 }, (_, i) => ({
        id: i + 1,
    ...
    const currentItems = useMemo(() => {
        return Divisions.slice(offset, offset + itemsPerPage);
    }, [currentPage]);
    ...
    const { isAddDivisionModalOpen, openAddDivisionModal, isEditDivisionModalOpen, openEditDivisionModal, isDeleteDivisionModalOpen, openDeleteDivisionModal } = useDivisionModal();

    return (
        <>
            {
                isAddDivisionModalOpen ? <AddDivisionModal /> : ""
            }
            {
                isEditDivisionModalOpen ? <EditDivisionModal /> : ""
            }
            {
                isDeleteDivisionModalOpen ? <DeleteDivisionModal /> : ""
            }
            <div className="w-full bg-white rounded-xs border border-[#ebebeb]">
                            ...
                            >
                                ...
                                <td className="px-5 py-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        onClick={openDeleteDivisionModal}
                                        >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 px-5 py-2">
                    ...
                    <p className="text-xs text-[#666666]">
                        Showing {offset + 1} to{" "}
                        {Math.min(offset + itemsPerPage, Divisions.length)} of{" "}
                        {Divisions.length} entries
                    </p>
