...
import { useCompanyModal } from "@/context/admin/company-context";
import AddCompanyModal from "@/components/Admin/Company/Modal/AddCompanyModal";
import EditCompanyModal from "@/components/Admin/Company/Modal/EditCompanyModal";
import DeleteCompanyModal from "@/components/Admin/Company/Modal/DeleteCompanyModal";
...
export default function CompanyTable() {
    ...
    const Companys = Array.from({ length: 1284 }, (_, i) => ({
        id: i + 1,
    ...
    const currentItems = useMemo(() => {
        return Companys.slice(offset, offset + itemsPerPage);
    }, [currentPage]);

    const pageCount = Math.ceil(Companys.length / itemsPerPage);

    const handlePageClick = (event: any) => {
        setCurrentPage(event.selected);
    };

    const { isAddCompanyModalOpen, openAddCompanyModal, isEditCompanyModalOpen, openEditCompanyModal, isDeleteCompanyModalOpen, openDeleteCompanyModal } = useCompanyModal();
    ...
    return (
        <>
            ...
            {
                isDeleteCompanyModalOpen ? <DeleteCompanyModal /> : ""
            }
            <div className="w-full bg-white rounded-xs border border-[#ebebeb]">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 px-5 py-2">
                    ...
                    <p className="text-xs text-[#666666]">
                        Showing {offset + 1} to{" "}
                        {Math.min(offset + itemsPerPage, Companys.length)} of{" "}
                        {Companys.length} entries
                    </p>
